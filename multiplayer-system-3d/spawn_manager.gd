...
func _add_player_to_game(network_id: int):
	...
	# pick random spawn
	if get_tree().current_scene.spawn_points.size() > 0:
		var index = randi() % get_tree().current_scene.spawn_points.size()
		var spawn_point = get_tree().current_scene.spawn_points[index]
