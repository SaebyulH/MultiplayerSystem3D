...
func get_cooldown_remaining(index: int) -> float:
	if index < 0 or index >= _cooldowns.size():
		return 0.0
	return maxf((_cooldowns[index] - Time.get_ticks_msec()) / 1000.0, 0.0)
...
func _start_cooldown(index: int, seconds: float) -> void:
	_cooldowns[index] = Time.get_ticks_msec() + int(seconds * 1000.0)
